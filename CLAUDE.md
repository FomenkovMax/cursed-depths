...
## Игровой контент живёт в двух местах — это не случайность, а конкретный критерий

Расы (`Race`), классы (`GameClass`) и способности (`Ability`) хранятся в Prisma-таблицах и
приходят через сид-скрипт (`prisma/seed.ts` → `src/lib/seed-data.ts`). Локации, предметы и враги
(`LOCATIONS`/`ITEMS`/`ENEMIES`) — статические массивы в `src/lib/game-data.ts`, версионируются в
git, никакой таблицы под них нет.

**Критерий, а не привычка:** `Player.raceId`/`Player.classId` — настоящие Prisma `@relation`
(`prisma/schema.prisma`, вокруг строки 28) — Prisma должна иметь таблицу на другом конце связи,
чтобы делать `include`/join и держать ссылочную целостность. `Player.locationId`, наоборот, —
голый `String` без `@relation`: игровая логика резолвит его в память через
`LOCATIONS.find(l => l.id === player.locationId)`, без похода в БД. То же самое для `itemId` в
`Inventory` и `enemyId` в бою — просто строки, сверяемые со статическими массивами.

**Практическое правило:** если новая сущность войдёт в Player (или другую модель) как настоящая
Prisma-связь (`@relation`, нужен join/`include`) — она должна жить в БД, как раса/класс. Если на
неё будут ссылаться только строковым id, резолвящимся в памяти без БД-запроса — она статический
массив в `game-data.ts`, как локация/предмет/враг. Не "открывается ли игроком" (расы тоже
открыты игроку с первой секунды) и не "баланс vs прогрессия" — а конкретно нужна ли Prisma
связь для этой сущности.

...
story.append(P('Мир был создан Четырьмя Столпами — божественными сущностями, каждая из которых воплотила фундаментальный аспект реальности. Веларион даровал Пламя воли и веры, Айлет сплела Нить Жизни, Торнак заложил незыблемую Основу, а Кессара создала Отражение — зеркало истины, испытующее души. Веками боги правили в гармонии, пока смертный по имени Карсус не совершил невозможное — попытался возвыситься до божественного статуса, использовав ритуал, связавший все четыре Столпа одновременно.'))
story.append(Spacer(1, 4))
story.append(P('Результат стал катастрофой мирового масштаба: Карсус не стал богом, а Падение разорвало ткань реальности. Четыре Столпа рухнули в Глубь — пространственный разлом, образовавшийся в момент раскола. Боги не умерли — они пали, сохранив искру сознания, но утратив контроль над своими дарами. Их сила, искажённая безумием и страданием, стала Скверной — тёмной энергией, пропитывающей Глубь и медленно поднимающейся на поверхность.'))
...
RACE_LORE = [
    ...
    ('ЛЮДИ — Хранитель Воли', [
        ('История', 'Люди — самая противоречивая раса. Именно человек Карсус вызвал Падение, и именно люди несут бремя вины. После Падения люди разделились: одни искали искупления через веру, другие — силы через тьму. Люди строят крепости из пепла и верят, что Воля сильнее проклятия.'),
        ('Столп', 'Веларион (Пламя) — люди тяготеют к воле и вере. Их сила — в убеждённости, которая может очищать или разрушать.'),
        ('Культура', 'Воля — главный ресурс. Человек без веры — пуст. Оружие священно, клятвы обязательны. Карсус — и проклятие, и предупреждение.'),
...
story.append(P('Тема: Руины человеческой цивилизации, последствия греха Карсуса. Игрок пробуждается у Пепельных Врат без памяти, узнаёт о Падении от НПС-наставника, выбирает первую специализацию (уровень 3), встречает представителя другой расы и обнаруживает Разлом — вход в Глубь.'))
story.append(Spacer(1, 4))
story.append(build_center_table(
...
ARCS = [
    ...
    ('Арка 3 (Глубь): Нисхождение', 'Путешествие вниз через зоны каждого Столпа. Каждая зона раскрывает историю одного бога. Игрок переживает их воспоминания. Понимает: Карсус не был злодеем — он видел угрозу, которую боги игнорировали.'),
...
story.append(build_table(
    ...
    [
        ...
        ['Дамиан', 'Человек', 'Воин-попутчик', 'Потомок Карсуса — несёт вину предка'],

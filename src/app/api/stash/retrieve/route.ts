...
export async function POST(req: NextRequest) {
  ...
  try {
    const { stashItemId } = await req.json();
    if (!stashItemId) return NextResponse.json({ error: 'Укажите stashItemId' }, { status: 400 });

    const player = await db.player.findUnique({ where: { telegramId: auth.telegramId } });
    if (!player) return NextResponse.json({ error: 'Персонаж не найден' }, { status: 404 });

    const item = await db.stashItem.findUnique({ where: { id: stashItemId } });
    if (!item || item.playerId !== player.id) return NextResponse.json({ error: 'Предмет не найден' }, { status: 404 });

    await db.$transaction(async (tx) => {
      const deleted = await tx.stashItem.deleteMany({ where: { id: stashItemId } });

    ...
    try {
      await db.$transaction(async (tx) => {
        const dec = await tx.accountVault.updateMany({
          where: { accountId: player.accountId!, gold: { gte: amount } },
          data: { gold: { decrement: amount } },
        });
        if (dec.count === 0) throw new Error('NOT_ENOUGH_GOLD');

        await tx.player.update({ where: { id: player.id }, data: { gold: { increment: amount } } });
      });
    } catch (e) {
      if (e instanceof Error && e.message === 'NOT_ENOUGH_GOLD') {
        return NextResponse.json({ error: 'Недостаточно золота в сейфе' }, { status: 400 });
      }
      throw e;
    }

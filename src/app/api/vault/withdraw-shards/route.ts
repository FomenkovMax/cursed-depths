  ...
  try {
    ...
    try {
      await db.$transaction(async (tx) => {
        const dec = await tx.accountVault.updateMany({
          where: { accountId: player.accountId!, crownShards: { gte: amount } },
          data: { crownShards: { decrement: amount } },
        });
        if (dec.count === 0) throw new Error('NOT_ENOUGH_SHARDS');

        await tx.player.update({ where: { id: player.id }, data: { crownShards: { increment: amount } } });
      });
    } catch (e) {
      if (e instanceof Error && e.message === 'NOT_ENOUGH_SHARDS') {
        return NextResponse.json({ error: 'Недостаточно Осколков Короны в сейфе' }, { status: 400 });
      }
      throw e;
    }
    ...
    return NextResponse.json({ message: `+${amount} 👑 из общего сейфа`, player: updated });
  } catch (error) {

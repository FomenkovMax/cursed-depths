 ...
 * last-bastion, forest-whisper) изначально (когда группового режима не было
 * вообще) трактовалась как бонус САМОМУ игроку — раздать эффект "нулю целей"
 * оставляло бы их decorative навсегда, а процент в тексте указан НА ОДНОГО
 * получателя ауры, так что self-применение не завышало силу относительно
 * замысла. Теперь, когда пати-бой реален (lib/party-combat-resolver.ts),
 * eternal-vow/sacred-aura/the-leader — настоящие ауры "союзники в группе/
 * рядом получают X": describesGroupPassiveAura() ниже отмечает их как такие,
 * и party-combat-resolver.ts фан-аутит РЕЗУЛЬТАТ ЭТОЙ ЖЕ функции
 * parsePassiveEffect (без изменений) на всю живую пати вместо одного
 * носителя — сама эта функция ничего не знает про пати и продолжает просто
 * разбирать текст, self-применение остаётся дефолтным поведением для
 * ОДИНОЧНОГО боя (единственный игрок и есть "вся пати" в этом контексте).
 * last-bastion и forest-whisper self-only НАВСЕГДА, не только пока нет пати:
 * "при защите союзника" (last-bastion) — это бонус САМОМУ носителю ПО ЕГО
 * ЖЕ действию "защита", не аура на союзников, реальных целей тут в принципе
 * не требуется; "вне боя ... за ход" (forest-whisper) тикает в /api/explore,
 * который вызывается per-player вне какого-либо инстанса — у пати нет общего
 * "хода вне боя", который можно было бы разделить на всех.
 *
...
/**
 * Пассивки-ауры "союзники в группе/рядом получают X" (eternal-vow, sacred-aura, the-leader) —
 * см. заголовок файла. Вызывающая сторона в пати-бою (party-combat-resolver.ts) использует этот
 * детектор, чтобы отличить их от остальных self-only эффектов, разобранных ТОЙ ЖЕ функцией
 * parsePassiveEffect выше (сама она не в курсе про пати и не меняется), и раздать результат
 * на всю живую пати, а не одному носителю. Не ловит last-bastion ("при защите союзника
 * собственная броня" — бонус самому носителю по его же действию, не аура) и forest-whisper
 * (внебоевой тик, не относится к пати-бою вообще, см. заголовок).
 */
export function describesGroupPassiveAura(description: string): boolean {
  return /союзники (в группе|рядом) получают/i.test(description);
}

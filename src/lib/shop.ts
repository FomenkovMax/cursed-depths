 ...
 *
 * Продать игрок может что угодно, кроме квестовых предметов и того, что
 * сейчас надето — по половине заявленной цены (value).
 */
...
/** Можно ли продать предмет с данным itemId магазину (всё, кроме квестовых). */
export function isSellableItem(itemId: string): boolean {
  const itemData = ITEMS.find(i => i.id === itemId);
  return !!itemData && itemData.type !== 'quest';
}
